-- directors ordered by avg. total gross with most frequent genre shown 
SELECT persons.id, 
       persons.full_name,
...
GROUP  BY persons.id, 
          ...
          film_persons.role 
HAVING Avg(financial_summary.total_gross) IS NOT NULL 
       AND film_persons.ROLE = 'Director' 
ORDER  BY Avg(financial_summary.total_gross) DESC;